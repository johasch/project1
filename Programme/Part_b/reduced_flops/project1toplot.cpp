...
int main(){
	...
	double h;
        ...
        double *binverse = new double[n+2];
	...
	//second
	for(int i=n;i>0;i--){
		btilde[i-1]=(btilde[i-1]+btilde[i])/b[i];	// normalization of the Koeficient (be careful, btilde is from now an not the original btilde)
	}
	...
	Zieldatei1.close();
	delete [] a, b, c, x, v, btilde,epsilon,binverse;
	}
